...
public class CompteMembre extends JPanel {
	 ...
	 * Create the panel.
	 */
	public CompteMembre(Membre m, DashBoard dashBoard) {
		...
		tableau.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent arg0) {
				...
				Date date = new Date();
				String dateInString = (String) tableau.getValueAt(ligne, 0);
				String type = (String) tableau.getValueAt(ligne, 1);
				Double valeur = (Double) tableau.getValueAt(ligne, 2);
				String status = (String) tableau.getValueAt(ligne, 3);
				...
				transaction = new Transaction(m.getMatricule(), date, valeur, type, paye);
		...
		btnPayer.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				lblError.setText("");
				if(transaction != null) {
					m.payerTransaction(transaction);
					dashBoard.changeScreen("compteMembre");
				}else {
	...
	public Object[][] dataJtable(Membre membre) {
		int nbrObject = membre.compte().size();
		Object[][] data = new Object[nbrObject][4];
		int i = 0;

		for (Transaction t : membre.compte()) {
			if(!t.isPayer()) {
				String date = new SimpleDateFormat("dd-MM-YYYY").format(t.getDateTransaction());
				data[i][0] = date;
				data[i][1] = t.getType();
				data[i][2] = t.getValeur();
				data[i][3] = "Non Payé";
			}
			i++;
		}
